...
export function Home() {
        ...
        if ('geolocation' in navigator) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    // Default to Oslo if denied/error (optional fallback)
    ...
    const handleSearch = (query: string) => {
        if (location) {
            ...
            // Navigate to results page with query and location state
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#fafafa]">
            ...
            {/* Hero Section */}
            <section className="relative pt-20 pb-24 px-4 sm:px-6 lg:px-8 overflow-hidden min-h-[85vh] flex items-center">
                {/* Morphing Gradient Orbs */}
                <div className="absolute inset-0 -z-20 bg-[#fafafa]">
                    <div
                        className="absolute top-[-5%] left-[-5%] w-[45%] h-[45%] bg-gradient-to-br from-primary/30 to-pink-400/20"
                        style={{ animation: 'morph 8s ease-in-out infinite, pulse-glow 4s ease-in-out infinite' }}
                    ></div>
                    <div
                        className="absolute top-[20%] right-[-10%] w-[35%] h-[35%] bg-gradient-to-bl from-secondary/25 to-cyan-400/15"
                        style={{ animation: 'morph 10s ease-in-out infinite 2s, pulse-glow 5s ease-in-out infinite 1s' }}
                    ></div>
                    <div
                        className="absolute bottom-[-10%] left-[30%] w-[30%] h-[30%] bg-gradient-to-tr from-accent/20 to-orange-300/10"
                ...
                <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
                    {['🥛', '🍞', '🧀', '🥕', '🍗', '🥚', '🍎', '🥦'].map((emoji, i) => (
                        <span
                            key={i}
                            className="absolute text-2xl sm:text-3xl select-none opacity-0"
                            style={{
                                left: `${10 + i * 11}%`,
                                bottom: '-40px',
                ...
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    <div className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full bg-white/70 backdrop-blur-md border border-black/5 mb-10 animate-reveal [animation-delay:100ms] [animation-fill-mode:forwards] shadow-sm">
                        <span className="flex h-2 w-2 rounded-full bg-green-500 animate-pulse"></span>
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-[0.2em]">Live Price Comparison</span>
                    </div>

                    <h1 className="text-5xl sm:text-6xl md:text-[5rem] font-heading font-extrabold text-dark mb-6 animate-reveal [animation-delay:200ms] [animation-fill-mode:forwards] leading-[1.1] tracking-tight whitespace-nowrap">
                    ...
                    <p className="text-lg sm:text-xl text-gray-400 mb-14 max-w-xl mx-auto animate-reveal [animation-delay:400ms] [animation-fill-mode:forwards] leading-relaxed font-medium">
                        {t('home.subtitle')}
                    </p>

                    <div className="animate-reveal [animation-delay:600ms] [animation-fill-mode:forwards] max-w-2xl mx-auto">
                        <SearchInput
                        ...
                        />
                        {locationError && (
                            <p className="mt-6 text-sm text-amber-600 glass px-6 py-2 rounded-full inline-block animate-reveal">
                    ...
                    {/* Quick-Search Chips */}
                    <div className="mt-8 animate-reveal [animation-delay:800ms] [animation-fill-mode:forwards]">
                            ...
                            {[
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 rounded-full text-sm font-semibold 
                                        bg-white/80 backdrop-blur-sm border border-gray-200 text-gray-600
